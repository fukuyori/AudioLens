...
)
{
    ...
    // AudioLens exposes a render endpoint only; see minipairs.h. The function
    // is kept so the device start sequence stays identical to the upstream
    // sample, which makes future diffs against it readable.
    UNREFERENCED_PARAMETER(_pDeviceObject);
