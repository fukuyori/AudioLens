...
--*/
{
    ...
    // Only the removed microphone path wrote a format here; the class handler
    // fills it in for the render pin.
    ...
    // The upstream sample special-cases its microphone array pin here, pinning
    // it to a single format. AudioLens has no capture endpoint, so only the
    // render path remains and the class handler does the work.
