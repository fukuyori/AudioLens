...
// AudioLens exposes a render endpoint only. The upstream sample also builds a
// microphone array capture endpoint; AudioLens reads its own output back with a
// WASAPI loopback client instead, so a capture endpoint would only clutter the
// user's recording device list.
...
//=============================================================================
//
// AudioLens registers no capture endpoints. C++ has no zero-length array, so
// the count is a literal and InstallAllCaptureFilters() returns immediately
// rather than iterating over an array that does not exist.
//
