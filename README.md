...
「会話」「講義」「映画」「深夜」「古い録音」などのプリセットを選ぶだけで、低音の濁りを抑え、人の声に必要な周波数帯を強調し、音量差を自動的に整える。専門知識は不要で、「低音」「声の明瞭さ」「音量差」の 3 項目で効果の強さを調整できる。
...
映画プリセットで、音量差(EBU R128 の LRA)が 17.99 → 5.26 LU に縮小することを実測済み。リアルタイム時の CPU 使用率は 1 コアあたり 0.55%、追加遅延は 2 ms。
...
src/dsp/               フィルタ、コンプレッサー、リミッター、DSP チェーン
